...
function GooUploader(Div,property,picId){
    this.$multiple=false;
    ...
    this.$swfUpload.uploadSuccess=function(file,msg){
    	var img='<img alt="图片" width="400px" src="UploadImages/'+msg+'">';
//    	var img='<img id="aa" width="200px" onclick="mainimg(\''+msg+'\')" src="UploadImages/'+msg+'" />&nbsp;&nbsp;&nbsp;&nbsp;';
    	img=img.replace(/(\r\n)+/g, "");
    	msg=msg.replace(/(\r\n)+/g, "");
    	$("#picshow"+picId).html(img);
    	  $("#signImg"+picId).val(msg);//显示上传成功结果 
        var id=file.id;
...
jQuery.extend({
	createGooUploader: function(Div,property,picId) {
		return new GooUploader(Div,property,picId);
  }
});